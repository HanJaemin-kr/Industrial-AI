...
def calculate_fitness(chromosomes, generator, discriminator, generation):
    ...
    for chromosome in chromosomes:
        ...
        train(generator, discriminator, latent_dim, generation, n_epochs=10, n_batch=128, n_eval=200)
    ...
    for i in range(n_epochs):
        ...
        if (i + 1) % n_eval == 0:
            ...
            with torch.no_grad():
                ...
                real_samples_cpu = real_samples_tensor.cpu()
                fft_real_samples = np.fft.fft(real_samples_cpu.numpy())

                gen_samples_tensor = torch.Tensor(gen_samples)
                gen_samples_cpu = gen_samples_tensor.cpu()
                fft_fake_samples = np.fft.fft(gen_samples_cpu.numpy())
                ...
                plt.subplot(1, 2, 1)
                plt.plot(gen_samples_cpu[0].numpy(), '-', label='Random Fake Sample', color='firebrick')
                plt.plot(real_samples_cpu[0].numpy(), '-', label='Random Real Sample', color='navy')
                plt.title('Signal')
                plt.legend(fontsize=10)

            plt.legend(fontsize=10)
            plt.tight_layout()
            plt.savefig(f'img2/ball-graph_g-{generation}_epoch-{i + 1}.png')
            plt.close()
...
def tournament_selection(chromosomes, fitness_scores, tournament_size):
    ...
    for _ in range(len(chromosomes)):
        ...
        tournament_indices = torch.randint(len(chromosomes), (tournament_size,))
        tournament_indices = tournament_indices.tolist()

        fitness_scores = torch.tensor(fitness_scores)
        tournament_fitness_scores = torch.index_select(fitness_scores, 0, torch.tensor(tournament_indices))

        # Select the fittest individual from the tournament
        selected_index = tournament_indices[torch.argmax(tournament_fitness_scores)]
    ...
    for chromosome in offspring:
        if torch.rand(1) < mutation_rate:
            ...
            for key in chromosome.keys():
                mutated_chromosome[key] = chromosome[key] + torch.randn_like(chromosome[key]) * 0.1
...
for generation in range(MAX_GENERATIONS):
    ...
    print('calculate fitness scores')
    fitness_scores = calculate_fitness(population, generator, discriminator, generation)
    print('perform tournament selection to choose parents')
    parents = tournament_selection(population, fitness_scores, tournament_size=5)
    print('perform crossover to create offspring')
    offspring = crossover(parents)
    print('perform mutation on the offspring')
    mutated_offspring = mutate(offspring, mutation_rate=0.1)
    print('replace the population with the new generation (offspring + mutated offspring) ')
    ...
    MODEL_PATH = "./model/model_ver_1.pth"
