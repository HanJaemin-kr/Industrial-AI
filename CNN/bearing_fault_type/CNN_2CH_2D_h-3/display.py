...
def make_dataset(bearing_normal_dataset, bearing_fault_dataset, fault_case, save_dir, repetitions):
    ...
    for make_file_count in range(setHeight, setting_make_file_count):
        ...
        axs[0, 0].plot(xlabel, normal_DE, color='blue')
        axs[0, 0].set_title('Normal_DE')
        axs[0, 1].plot(xlabel, normal_NDE, color='green')
        axs[0, 1].set_title('Normal_NDE')

        # Plot fault data
        axs[1, 0].plot(xlabel, fault_DE, color='red')
        axs[1, 0].set_title(fault_case + ' DE')
        axs[1, 1].plot(xlabel, fault_NDE, color='orange')
        axs[1, 1].set_title(fault_case + ' NDE')
        ...
        # Save the image
        plt.savefig(os.path.join(save_dir, f'image_{make_file_count}.png'))
        plt.close()

    return f"{save_dir} Success: {repetitions} images saved"
