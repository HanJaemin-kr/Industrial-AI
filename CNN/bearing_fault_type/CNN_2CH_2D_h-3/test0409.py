...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            tensor = self.transform(tensor)
        # Add Gaussian noise
        # noise = torch.randn_like(tensor) * 0.1  # Adjust std_dev as needed
        # tensor += noise
        return tensor, label
...
def load_model(target_name):
    ...
    model = Net()
    model.load_state_dict(torch.load( f"{target_name}"))
...
def __main__():
    ...
    model_dir = 'CNN\\bearing_fault_type\\CNN_2CH_2D_h-3\\model0416'
    pt_files = [file for file in os.listdir(model_dir) if file.endswith('.pt')]
    #pt_files = ['data_model_6915_1000_4.pt', 'data_model_618_1000_4.pt']
    ...
    for model_name in pt_files:
        model, device = load_model( f"{model_dir}/{model_name}")
